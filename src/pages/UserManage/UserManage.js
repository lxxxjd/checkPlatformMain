    ...
    >
      <Row>
        <Col span={13}>
          ...
          <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 15 }} label="所在公司：" colon={false}>
            <span className="ant-form-text">{modalInfo.companyname}</span>
          </FormItem>
...
@connect(({ clientuser, loading }) => ({
  clientuser,
  loading: loading.models.clientuser,
}))
@Form.create()
class UserManage extends PureComponent {
  ...
  checkUserNameFetch = (rule, value, callback) => {
    ...
    dispatch({
      type: 'clientuser/checkUserNameFetch',
  ...
  modifyItem = text => {
    ...
    dispatch({
      type: 'clientuser/getDepartmentList',
      payload: params,
      callback: (response) => {
        if (response){
  ...
  render() {
    ...
    const {  modalVisible,modalInfo,addModalVisible,dataSource, previewVisible, signUrl,departments} = this.state;
